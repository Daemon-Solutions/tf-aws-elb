resource "aws_elb" "elb" {
  ...
  listener {
    instance_port      = "${var.backend_port3}"
    instance_protocol  = "${var.backend_proto3}"
    lb_port            = "${var.elb_port3}"
    lb_protocol        = "${var.elb_proto3}"
    ssl_certificate_id = "${var.ssl_cert_id3}"
  }

  listener {
    instance_port      = "${var.backend_port4}"
    instance_protocol  = "${var.backend_proto4}"
    lb_port            = "${var.elb_port4}"
    lb_protocol        = "${var.elb_proto4}"
    ssl_certificate_id = "${var.ssl_cert_id4}"
  }
